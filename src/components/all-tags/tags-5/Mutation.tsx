...
import type { IParagraphData, ParaSize, ParaPadding, ParaWeight } from './data';
import { defaultDataParagraph } from './data';
...
const SIZE_MAP: Record<ParaSize, string> = {
  xs: 'text-xs',
  sm: 'text-sm',
  base: 'text-base',
  lg: 'text-lg',
...
const WEIGHT_MAP: Record<ParaWeight, string> = {
  light: 'font-light',
  normal: 'font-normal',
  medium: 'font-medium',
...
const PADDING_MAP: Record<ParaPadding, string> = {
  none: 'p-0',
  xs: 'p-2',
  sm: 'p-4',
  md: 'p-6',
...
const MutationParagraph = ({ data, onSubmit }: ParagraphFormProps) => {
  const [formData, setFormData] = useState<IParagraphData>({ ...defaultDataParagraph });

  useEffect(() => {
    if (data) {
      setFormData({ ...defaultDataParagraph, ...data });
    }
  }, [data]);
  ...
  const handleReset = () => {
    setFormData(defaultDataParagraph);
  };
      ...
      <div className="lg:w-1/2 bg-zinc-950 p-6 lg:p-8 flex flex-col h-full overflow-y-auto">
        <div className="space-y-8">
           <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent mb-1">
                Edit Paragraph
              </h2>
          ...
          <div className="space-y-4">
            <Label className="text-zinc-400 flex items-center gap-2"><Pilcrow size={14}/> Content</Label>
            <Textarea
              value={formData.text}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
               <Label className="text-zinc-400 text-xs uppercase tracking-wider font-bold flex items-center gap-2"><Type size={14}/> Typography</Label>
            ...
            <div className="space-y-2">
               <Label className="text-zinc-400 text-xs uppercase tracking-wider font-bold flex items-center gap-2"><MoveVertical size={14}/> Layout & Style</Label>
               ...
               <div className="flex bg-zinc-900/50 p-1 rounded-xl border border-zinc-800 mb-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="flex items-center justify-between p-3 bg-zinc-900/30 border border-zinc-800 rounded-lg">
                      <span className="text-sm text-zinc-300 flex items-center gap-2"><UnderlineIcon size={14}/> Underline</span>
            ...
            <div className="space-y-3">
               <div className="flex justify-between">
                 <Label className="text-zinc-400 text-xs uppercase tracking-wider font-bold flex items-center gap-2"><Droplets size={14}/> Opacity</Label>
                 <span className="text-xs text-emerald-400 font-mono">{formData.opacity}%</span>
               </div>
               <Slider
                  value={[formData.opacity]}
                  onValueChange={([val]) => updateField('opacity', val)}
                  max={100}
                  step={5}
                  className="py-2"
                />
...
export default MutationParagraph;

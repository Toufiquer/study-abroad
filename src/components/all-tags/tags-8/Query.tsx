...
import { defaultDataTagSlider, ITagSliderData, TagSliderProps } from './data';

const QueryTagSlider = ({ data }: TagSliderProps) => {
  let sliderData = defaultDataTagSlider;
  ...
  // Ensure we don't divide by zero or have invalid items
  const safeItemsPerSlide = itemsPerSlide || 4;

  // Calculations for translation
  const nextSlide = useCallback(() => {
    setCurrentIndex(prev => {
      // If we have fewer items than the visible slot, no need to slide much,
      // but logic handles standard carousel behavior:
  ...
  useEffect(() => {
    ...
    // If content fits entirely, maybe disable autoplay?
    // Optional: if (totalItems <= safeItemsPerSlide) return;

...
// --- Types ---
export type NavPosition = 'hidden' | 'middle-outside' | 'bottom-outside';
...
export const defaultDataLogo: ILogoData = {
  logos: [
    { id: '1', image: '', alt: 'Company A', link: '' },
    { id: '2', image: '', alt: 'Company B', link: '' },
    { id: '3', image: '', alt: 'Company C', link: '' },
    { id: '4', image: '', alt: 'Company D', link: '' },
  ],
  autoplaySpeed: 2500,
  isAutoplay: true,
  infiniteLoop: true,
  ...
  <div className="space-y-1.5">
    ...
    <div className="relative group">
      ...
      <Input
        {...props}
        className={cn(
          'w-full bg-zinc-900/50 border-zinc-800 focus:border-blue-500',
          Icon ? 'pl-9 pr-3' : 'px-3',
        )}
      />
    </div>
...
/**
 * Device Breakpoint Card
 */
const DeviceControl = ({ icon: Icon, label, value, onChange, min, max }: { icon: React.ElementType; label: string; value: number; onChange: (val: number) => void; min: number; max: number }) => (
...
const MutationLogo = ({ data, onSubmit }: LogoFormProps) => {
  const [formData, setFormData] = useState<ILogoData>({ ...defaultDataLogo });
  const [activeLogoIndex, setActiveLogoIndex] = useState<number>(0);
  const [activeTab, setActiveTab] = useState<'assets' | 'settings'>('assets');

  useEffect(() => {
    if (data) setFormData({ ...defaultDataLogo, ...data });
  }, [data]);
                ...
                <section className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div
                        className="flex items-center justify-between p-3 bg-zinc-900/30 border border-zinc-800 rounded-xl cursor-pointer"
                      >

...
import MutationSection1 from '@/components/all-section/section-1/Mutation';
import MutationSection2 from '@/components/all-section/section-2/Mutation';
import MutationSection3 from '@/components/all-section/section-3/Mutation';
import MutationSection4 from '@/components/all-section/section-4/Mutation';
import MutationSection5 from '@/components/all-section/section-5/Mutation';
import MutationSection6 from '@/components/all-section/section-6/Mutation';

import { ScrollArea } from '@/components/ui/scroll-area';
import { ISectionData } from '@/components/all-section/section-2/data';
...
export const SectionCard = ({ section }: SectionCardProps) => {
  ...
  const handleSectionEdit = (updatedValue: SectioinDataType) => {
    updateSection(section._id, { ...section, content: updatedValue });
  ...
  const getDefaultImageUrl = (sectionId: string): string => {
    switch (sectionId) {
      case 'section-uid-1':
        return '/all-section/section-1.png';
      case 'section-uid-2':
        return '/all-section/section-2.png';
      case 'section-uid-3':
        return '/all-section/section-3.png';
      case 'section-uid-4':
        return '/all-section/section-4.png';
      case 'section-uid-5':
        return '/all-section/section-5.png';
      case 'section-uid-6':
        return '/all-section/section-6.png';
      default:
        return '/all-section/section-1.png';
    }
  };
    ...
    <>
      ...
      <div ref={setNodeRef} style={style} className="group h-[300px] flex rounded-xl border border-white/20 bg-white/5 backdrop-blur-md">
        <div className="w-[70%] relative">
          {section.picture ? (
            ...
            <Image
              src={getDefaultImageUrl(section.sectionUid)}
              alt={section.title || 'section imgaes'}
        ...
        <div className="w-[30%] flex flex-col p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Switch checked={section.isActive} onCheckedChange={() => toggleActive(section._id)} />
          </div>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogPortal>
          ...
          <DialogContent className="z-[101] bg-slate-900/30 backdrop-blur-[8px] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 text-slate-100 shadow-2xl w-6xl max-w-6xl min-w-6xl h-[90vh]">
            <DialogHeader>
              <DialogTitle>Edit Section</DialogTitle>
              <DialogDescription className="text-white/70">Update the section data below</DialogDescription>
            </DialogHeader>

            <ScrollArea className="w-full h-[70vh] pr-4">{editComponent}</ScrollArea>

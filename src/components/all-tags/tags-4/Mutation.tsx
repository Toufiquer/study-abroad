...
// --- Constants & Maps ---
const SIZE_MAP: Record<DescSize, string> = {
  xs: 'text-xs',
  sm: 'text-sm',
  base: 'text-base',
  lg: 'text-lg',
...
const PADDING_MAP: Record<DescPadding, string> = {
  none: 'p-0',
  xs: 'p-2',
  sm: 'p-4',
  md: 'p-6',
...
const MutationDescription = ({ data, onSubmit }: DescriptionFormProps) => {
  const [formData, setFormData] = useState<IDescriptionData>({ ...defaultDataDescription });
  const editorRef = useRef<HTMLDivElement>(null);
  const [isFocused, setIsFocused] = useState(false);

  useEffect(() => {
    if (data) {
      setFormData({ ...defaultDataDescription, ...data });
  ...
  const handleReset = () => {
    setFormData(defaultDataDescription);
    if (editorRef.current) editorRef.current.innerHTML = defaultDataDescription.content;
  };
...
export default MutationDescription;

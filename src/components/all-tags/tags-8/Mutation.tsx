...
export const defaultDataTagSlider: ITagSliderData = {
  tags: [
    { id: '1', text: 'Design', link: '#' },
    { id: '2', text: 'Development', link: '#' },
    { id: '3', text: 'Marketing', link: '#' },
    { id: '4', text: 'Strategy', link: '#' },
  ],
  itemsPerSlide: 4,
  tagStyle: 'glassy',
  autoplaySpeed: 3000,
...
// --- Style Mappings ---
const STYLE_PRESETS: Record<TagStyle, string> = {
...
const ModernInput = ({ icon: Icon, ...props }: { icon?: React.ElementType } & React.InputHTMLAttributes<HTMLInputElement>) => (
  <div className="relative group flex-1">
    ...
    <Input
      {...props}
      className={cn(
        'w-full bg-zinc-950/50 border-zinc-800 focus:border-blue-500',
        Icon ? 'pl-9 pr-3' : 'px-3',
      )}
    />
  </div>
...
const MutationTagSlider = ({ data, onSubmit }: TagSliderFormProps) => {
  const [formData, setFormData] = useState<ITagSliderData>({ ...defaultDataTagSlider });
  const [activeTab, setActiveTab] = useState<'content' | 'settings'>('content');

  useEffect(() => {
    if (data) setFormData({ ...defaultDataTagSlider, ...data });
  }, [data]);
  ...
  const handleReset = () => {
    setFormData(defaultDataTagSlider);
  };
                    ...
                    {formData.tags.map((tag, index) => (
                      ...
                      >
                        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <ModernInput icon={Tag} value={tag.text} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateTag(index, 'text', e.target.value)} placeholder="Label" />
                          <ModernInput icon={LinkIcon} value={tag.link} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateTag(index, 'link', e.target.value)} placeholder="#" />
                        </div>
                        <button
...
export default MutationTagSlider;

...
export interface ISliderData {
  ...
  navPosition: 'middle-inside' | 'middle-outside' | 'bottom-overlay' | 'bottom-outside';
  showArrowsOnHover: boolean;
}

export const defaultDataSlider: ISliderData = {
  slides: [{ id: '1', image: '', title: 'Welcome', description: 'Start your journey here.' }],
  autoplaySpeed: 3000,
  isAutoplay: true,
  infiniteLoop: true,
  pauseOnHover: true,
 ...
 */
const ModernInput = ({ label, value, onChange, placeholder, type = 'text' }: { label: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder?: string; type?: string }) => (
  <div className="space-y-1.5">
    ...
    <Input
      ...
      className="bg-zinc-900/50 border-zinc-800 focus:border-blue-500"
    />
  </div>
);
...
const MutationSlider = ({ data, onSubmit }: SliderFormProps) => {
  const [formData, setFormData] = useState<ISliderData>({ ...defaultDataSlider });
  const [activeSlideIndex, setActiveSlideIndex] = useState<number>(0);
  const [activeTab, setActiveTab] = useState<'content' | 'settings'>('content');

  useEffect(() => {
    if (data) {
      setFormData({ ...defaultDataSlider, ...data });
    }
  }, [data]);
              ...
              <div className="space-y-6">
                ...
                {currentSlideData ? (
                  <motion.div key={currentSlideData.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                    ...
                    <ModernInput label="Heading" value={currentSlideData.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSlide(activeSlideIndex, 'title', e.target.value)} />

...
export type HoverEffect = 'none' | 'zoom' | 'overlay' | 'grayscale' | 'lift';
export type AnimationType = 'none' | 'fade' | 'scale' | 'slide-up';
...
export const defaultDataGallery: IGalleryData = {
  layout: 'grid',
  images: [
    { id: '1', url: '', caption: 'Project Alpha' },
    { id: '2', url: '', caption: 'Brand Identity' },
    { id: '3', url: '', caption: 'Marketing Asset' },
    { id: '4', url: '', caption: 'Team Photo' },
  ],
  columns: 3,
  gap: 'md',
  aspectRatio: 'square',
...
const MutationGallery = ({ data, onSubmit }: GalleryFormProps) => {
  const [formData, setFormData] = useState<IGalleryData>({ ...defaultDataGallery });
  const [activeImageIndex, setActiveImageIndex] = useState<number>(0);
  const [activeTab, setActiveTab] = useState<'content' | 'settings'>('content');

  useEffect(() => {
    if (data) setFormData({ ...defaultDataGallery, ...data });
  }, [data]);
  ...
  const handleReset = () => {
    setFormData(defaultDataGallery);
  };
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 flex-wrap pb-2 scrollbar-hide">
                    {formData.images.map((img, idx) => (
                      ...
                      >
                        {img.url ? (
                          <Image alt={img.caption} width={200} height={200} src={img.url} className="w-full h-full object-cover" />
                        ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        disabled={formData.layout === 'masonry'}
                      >
                        <SelectTrigger className="bg-zinc-900 border-zinc-800">
                          <SelectValue />
                        </SelectTrigger>
...
export default MutationGallery;

...
// --- Types ---
export type ImageWidth = 'auto' | '100%' | '75%' | '50%' | 'fixed-sm' | 'fixed-md' | 'fixed-lg';
...
const MutationImages = ({ data, onSubmit }: ImagesFormProps) => {
  const [formData, setFormData] = useState<IImagesData>({ ...defaultDataImages });

  useEffect(() => {
    if (data) setFormData({ ...defaultDataImages, ...data });
  }, [data]);
